...
/*   Updated: 2023/05/13 11:26:05 by lilizarr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	runpipe(t_pipex *data, char **argv, pid_t pid)
{
	...
	{
		// ft_putendl_fd(ft_itoa(waitpid(data->pid, &status, WUNTRACED)), 2);
		if (waitpid(-1, &status, 0) == data->pid)
			ft_putendl_fd("child exited:", 2);
...
{	
	...
	{
		// ft_putendl_fd(ft_itoa(waitpid(-1, &status, WUNTRACED)), 2);
